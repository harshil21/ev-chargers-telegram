...
async def check_if_user_is_authenticated(
...
async def authenticate_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in db and "is_auth" in db[user_id] and db[user_id]["is_auth"]:
        ...
        logging.info(
            f"User {update.effective_user.name} is already authenticated when using /auth"
        )
        raise ApplicationHandlerStop
    ...
    if len(args) != 2:
        ...
        logging.info(
            f"User {update.effective_user.name} used /auth with invalid arguments"
        )
        raise ApplicationHandlerStop
    ...
    if username == "admin" and password == "allowed":
        ...
        logging.info(
            f"User {update.effective_user.name} authenticated successfully using /auth"
        )
    ...
    # Invalid credentials
        ...
        logging.info(
            f"User {update.effective_user.name} failed to authenticate using /auth"
        )
